    ...
    <div className="relative overflow-hidden bg-white rounded-xl shadow-sm border border-slate-200 text-center">
      ...
      {isEnToEs ? (
        <>
          <div className="flex items-center justify-center gap-3">
            <h2 className="font-display text-2xl sm:text-3xl font-black text-ink">{word.englishTerm}</h2>
      ...
      <div className="mt-8 pt-6 border-t border-slate-100 min-h-[7rem] flex flex-col items-center justify-center">
        {!revealed ? (
          ...
          <button
            onClick={onReveal}
            className="flex items-center justify-center gap-2 rounded-lg border border-dashed border-slate-300 bg-slate-50 px-6 py-3 text-sm font-medium text-slate-500 hover:bg-primary/5 hover:text-primary hover:border-primary/50 transition-colors"
          >
          ...
          <>
            <div className="flex items-center justify-center gap-3">
              <p className="font-display text-xl font-bold text-primary-dark">{word.englishTerm}</p>
              {speakButton(word.englishTerm, "Escuchar palabra")}
            </div>
            {word.pronunciation && <Pronunciation ipa={word.pronunciation} />}
            {exampleBlock}
