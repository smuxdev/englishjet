...
### 25. Backend multi-usuario: Turso + Vercel Functions (2026-08-26)
- **Problema**: en producción no había escritura de contenido (CSV solo-dev) ni cuentas; el progreso moría con el navegador
- **Solución**: backend opcional con **Turso (libSQL)** y functions en `api/` (firma web `Request→Response`, exports por método `GET/POST/...`). Esquema en `db/schema.sql` (idempotente, `npm run db:migrate`; local = SQLite `file:.data/dev.db`, sin CLI de Turso): `users`, `sessions` (token opaco: en BD solo su sha256; cookie `ej_session` httpOnly/SameSite=Lax, 30 días), `cards` **por usuario con id sintético** (editar el término ya no cambia el id; box/due/my_sentence/examples JSON viven en la fila; `UNIQUE(user_id, english_term COLLATE NOCASE)`), `activity`, `login_attempts` (rate-limit 10 fallos/15 min por email, en la propia BD)
- **Auth propia**: scrypt de `node:crypto` (N=16384, parámetros embebidos en el hash), 401 genérico, registro con código de invitación (`REGISTRATION_CODE`; vacío = abierto). Anti-CSRF: SameSite=Lax + Content-Type JSON obligatorio en mutaciones. Anti-IDOR: toda query lleva `WHERE user_id`; id ajeno → 404 indistinguible. `withHandler` (api/_lib/log.ts): una línea JSON por request + 500 opaco
- **Modo dual del cliente**: interfaz `DeckStore` (src/services/deckStore.ts) con `localDeckStore` (comportamiento histórico intacto: CSV + localStorage, edición solo-dev) y `remoteDeckStore` (API; CRUD write-through, progreso/actividad/frase propia **optimistas** fire-and-forget para no bloquear la sesión de estudio). `VocabularyProvider` recibe el store por props y App lo **remonta con `key={user.id ?? "anon"}`** al cambiar de identidad. `activity.ts` refactorizado a transiciones puras (`applyReview`/`applyUnreview`); mySentences pasó de localStorage directo en StudySession al contexto (`setMySentence`)
- **Mazo por usuario**: empieza vacío; botón «Importar mazo de ejemplo» (638 palabras) — `initializeWords()` ya fusiona el progreso anónimo de localStorage, así que la migración a la cuenta viene gratis; `INSERT OR IGNORE` en lote (`cards/import`, máx 5000/2 MB)
- **DX**: plugin `devApi()` en vite.config.ts monta los handlers de `api/` como middleware vía `ssrLoadModule` (hot reload, sin `vercel dev`); los 3 endpoints CSV dev-only mantienen prioridad. `.env.local` (TURSO_DATABASE_URL/TURSO_AUTH_TOKEN/REGISTRATION_CODE) inyectado con `loadEnv`. tsconfig.api.json nuevo (moduleResolution bundler = esbuild de Vercel). Smoke test E2E por curl en verde: registro/dup/415, me, IDOR 404×2, import con skip de duplicados, activity con clamp del deshacer, logout, 429 al 11º fallo

### 26. Pantalla de bienvenida: la app queda detrás del login (2026-08-26)
- **Problema**: sin sesión se podía usar la app con el mazo por defecto; se quiere una puerta de entrada que invite a entrar/crear cuenta
- **Solución**: `WelcomeScreen.tsx` (hero de marca + `AuthForm.tsx`, extraído de AuthModal para reutilizarlo). Enrutado en App: con backend y sin sesión → bienvenida; con sesión → app remota. El **modo local sobrevive en sus dos contextos legítimos**: (a) build estático sin functions — `AuthProvider` distingue el 401 JSON limpio de `/api/auth/me` (hay backend, sin sesión) de 404/HTML/fallo de red (`backendAvailable=false` → modo local directo, UI de cuentas oculta); (b) dev — la bienvenida ofrece «continuar sin cuenta» si `/api/csv-editable` responde 204 (flujo de edición del CSV). Logout en prod → bienvenida; en dev-bypass → modo local
